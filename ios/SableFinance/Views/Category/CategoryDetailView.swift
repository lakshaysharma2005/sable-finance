    ...
    private func transactionsSection(_ data: CategoryData) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            ForEach(data.groups) { group in
                monthSection(group, categoryName: data.name)
            }
        }
    }

    private func monthSection(_ group: MonthGroup, categoryName: String) -> some View {
        // Net spend: expenses minus refunds/credits.
        let monthTotal = group.items.reduce(0.0) { sum, tx in sum + tx.amount }
